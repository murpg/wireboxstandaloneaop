...
wirebox.system.validation.result.IValidationResult function getValidationResults(){}

These methods are only active if WireBox entity injection is available.


*/
component extends="wirebox.system.orm.hibernate.VirtualEntityService" accessors="true"{
	...
	*/
	function init(string queryCacheRegion, boolean useQueryCaching,	boolean eventHandling, boolean useTransactions,	boolean defaultAsQuery){
		...
		// datasource
		arguments.datasource = new wirebox.system.orm.hibernate.util.ORMUtilFactory().getORMUtil().getEntityDatasource( this );
	...
	*/
	wirebox.system.validation.result.IValidationResult function getValidationResults(){
		...
		return new wirebox.system.validation.result.ValidationResult();
	}
