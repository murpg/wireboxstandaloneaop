...
component accessors="true"{
	...
	*/
	...
	BaseORMService function init(string queryCacheRegion="ORMService.defaultCache",
		...
		if( directoryExists( expandPath("/wirebox") ) ){
			ORMEventHandler = new wirebox.system.orm.hibernate.WBEventHandler();
		}
		else{
			ORMEventHandler = new wirebox.system.orm.hibernate.EventHandler();
		}

		// Create the ORM Utility component
		orm = new wirebox.system.orm.hibernate.util.ORMUtilFactory().getORMUtil();

		// Create our bean populator utility
		beanPopulator = createObject("component","wirebox.system.core.dynamic.BeanPopulator").init();

		// Restrictions orm.hibernate.criterion.Restrictions
		restrictions = createObject("component","wirebox.system.orm.hibernate.criterion.Restrictions").init();
	...
	*/
	any function createService(required string entityName,
		...
		return  CreateObject("component", "wirebox.system.orm.hibernate.VirtualEntityService").init(argumentCollection=arguments);
	}
