...
<!------------------------------------------- CONSTRUCTOR ------------------------------------------->
	...
	<cfscript>
		...
		this.logLevels = createObject("component","wirebox.system.logging.LogLevels");
	...
	<cffunction name="init" access="public" returntype="LogBox" hint="Constructor" output="false" >
		<cfargument name="config"  type="wirebox.system.logging.config.LogBoxConfig" required="true" hint="The LogBoxConfig object to use to configure this instance of LogBox"/>
	...
	<cffunction name="configure" output="false" access="public" returntype="void" hint="Configure logbox for operation. You can also re-configure LogBox programmatically. Basically we register all appenders here and all categories">
		<cfargument name="config" type="any" required="true" hint="The LogBoxConfig object to use to configure this instance of LogBox: wirebox.system.logging.config.LogBoxConfig" colddoc:generic="wirebox.system.logging.config.LogBoxConfig"/>
		<cfscript>
			var appenders 	= "";
			var key 		= "";
			var oRoot 		= "";
		...
		<cflock name="#instance.logboxID#.logBox.config" type="exclusive" timeout="30">
			...
			oRoot = createObject("component","wirebox.system.logging.Logger").init(argumentCollection=args);
	...
	<cffunction name="getConfig" access="public" returntype="any" output="false" hint="Get this LogBox's configuration object." colddoc:generic="wirebox.system.logging.config.LogBoxConfig">
...
<!------------------------------------------- PUBLIC ------------------------------------------->
	...
	<!--- getRootLogger --->
	<cffunction name="getRootLogger" output="false" access="public" returntype="any" hint="Get the root logger" colddoc:generic="wirebox.system.logging.Logger">
		<cfreturn instance.loggerRegistry["ROOT"]>
	</cffunction>

	<!--- getLogger --->
	<cffunction name="getLogger" output="false" access="public" returntype="any" hint="Get a logger object configured with a category name and appenders. If not configured, then it reverts to the root logger defined for this instance of LogBox" colddoc:generic="wirebox.system.logging.Logger">
		...
		<cflock name="#instance.logboxID#.logBox.logger.#arguments.category#" type="exclusive" throwontimeout="true" timeout="30">
			<cfscript>
				if( NOT structKeyExists(instance.loggerRegistry,arguments.category) ){
					// Create logger	
					oLogger = createObject("component","wirebox.system.logging.Logger").init(argumentCollection=args);
					// Inject Root Logger
...
<!------------------------------------------- PRIVATE ------------------------------------------>
	...
	<!--- Get ColdBox Util --->
	<cffunction name="getUtil" access="private" output="false" returntype="any" hint="Create and return a util object" colddoc:generic="wirebox.system.core.util.Util">
