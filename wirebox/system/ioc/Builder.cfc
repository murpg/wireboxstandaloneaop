	...
	<cffunction name="init" access="public" returntype="Builder" hint="Constructor. If called without a configuration binder, then WireBox will instantiate the default configuration binder found in: wirebox.system.ioc.config.DefaultBinder" output="false" >
		...
		<cfscript>
			...
			if( instance.injector.isCacheBoxLinked() ){
				instance.cacheBoxDSL = createObject("component","wirebox.system.ioc.dsl.CacheBoxDSL").init( arguments.injector );
			}
			// Build LogBox DSL Namespace
			instance.logBoxDSL = createObject("component","wirebox.system.ioc.dsl.LogBoxDSL").init( arguments.injector );
	...
	<cffunction name="getProviderDSL" access="private" returntype="any" hint="Get dependencies using the our provider pattern DSL" output="false" >
		...
		<cfscript>
			...
			if( instance.injector.containsInstance( providerName ) ){
				...
				return createObject("component","wirebox.system.ioc.Provider").init(instance.injector.getScopeRegistration(), instance.injector.getScopeStorage(), providerName);
			}
			else if( instance.log.canDebug() ){
