...
----------------------------------------------------------------------->
<cfcomponent hint="A WireBox Injector: Builds the graphs of objects that make up your application." output="false" serializable="false" implements="wirebox.system.ioc.IInjector">

<!----------------------------------------- CONSTRUCTOR ------------------------------------->

	<!--- init --->
	<cffunction name="init" access="public" returntype="Injector" hint="Constructor. If called without a configuration binder, then WireBox will instantiate the default configuration binder found in: wirebox.system.ioc.config.DefaultBinder" output="false" >
		<cfargument name="binder" 		required="false" default="wirebox.system.ioc.config.DefaultBinder" hint="The WireBox binder or data CFC instance or instantiation path to configure this injector with">
		...
		<cfargument name="coldbox" 		required="false" default="" hint="A coldbox application context that this instance of WireBox can be linked to, if not using it, we just ignore it." colddoc:generic="wirebox.system.web.Controller">
		<cfscript>
			// Setup Available public scopes
			this.SCOPES = createObject("component","wirebox.system.ioc.Scopes");
			// Setup Available public types
			this.TYPES = createObject("component","wirebox.system.ioc.Types");

			// Do we have a binder?
			if( NOT len(trim(arguments.binder)) ){ arguments.binder = "wirebox.system.ioc.config.DefaultBinder"; }
			...
			instance = {
				...
				// Utility class
				utility  = createObject("component","wirebox.system.core.util.Util"),
				// Scope Storages
				scopeStorage = createObject("component","wirebox.system.core.collections.ScopeStorage").init(),
				// Version
				version  = "1.6.0",
	...
	<cffunction name="configure" output="false" access="public" returntype="void" hint="Configure this injector for operation, called by the init(). You can also re-configure this injector programmatically, but it is not recommended.">
		<cfargument name="binder" 		required="true" hint="The configuration binder object or path to configure this Injector instance with" colddoc:generic="wirebox.system.ioc.config.Binder">
		...
		<cflock name="#instance.lockName#" type="exclusive" timeout="30" throwontimeout="true">
			...
			instance.builder = createObject("component","wirebox.system.ioc.Builder").init( this );
    	...
    	<cflock name="Injector.RegisterNewInstance.#hash(arguments.instancePath)#" type="exclusive" timeout="20" throwontimeout="true">
    		<cfscript>
				if( NOT instance.binder.mappingExists( arguments.name ) ){
					...
					mapping = createObject("component","wirebox.system.ioc.config.Mapping")
						.init( arguments.name )
    ...
    <cffunction name="autowire" output="false" access="public" returntype="any" hint="I wire up target objects with dependencies either by mappings or a-la-carte autowires">
		...
		<cfargument name="mapping" 				required="false" 	hint="The object mapping with all the necessary wiring metadata. Usually passed by scopes and not a-la-carte autowires" colddoc:generic="wirebox.system.ioc.config.Mapping"/>
    ...
    <cffunction name="processMixins" output="false" access="private" returntype="void" hint="Process mixins on the selected target">
		...
		<cfscript>
			var mixin 	= createObject("component","wirebox.system.ioc.config.Mixin").$init( arguments.mapping.getMixins() );
			var key		= "";
    ...
    <cffunction name="setParent" output="false" access="public" returntype="void" hint="Link a parent Injector with this injector">
    	<cfargument name="injector" required="true" hint="A WireBox Injector to assign as a parent to this Injector" colddoc:generic="wirebox.system.ioc.Injector">
    	<cfset instance.parent = arguments.injector>
    </cffunction>

	<!--- getParent --->
    <cffunction name="getParent" output="false" access="public" returntype="any" hint="Get a reference to the parent injector instance, else an empty simple string meaning nothing is set" colddoc:generic="wirebox.system.ioc.Injector">
    	<cfreturn instance.parent>
    </cffunction>

	<!--- getObjectPopulator --->
    <cffunction name="getObjectPopulator" output="false" access="public" returntype="any" hint="Get an object populator useful for populating objects from JSON,XML, etc." colddoc:generic="wirebox.system.core.dynamic.BeanPopulator">
    	<cfreturn createObject("component","wirebox.system.core.dynamic.BeanPopulator").init()>
    </cffunction>

	<!--- getColdbox --->
    <cffunction name="getColdbox" output="false" access="public" returntype="any" hint="Get the instance of ColdBox linked in this Injector. Empty if using standalone version" colddoc:generic="wirebox.system.web.Controller">
	...
	<!--- getCacheBox --->
    <cffunction name="getCacheBox" output="false" access="public" returntype="any" hint="Get the instance of CacheBox linked in this Injector. Empty if using standalone version" colddoc:generic="wirebox.system.cache.CacheFactory">
	...
	<!--- getLogBox --->
    <cffunction name="getLogBox" output="false" access="public" returntype="any" hint="Get the instance of LogBox configured for this Injector" colddoc:generic="wirebox.system.logging.LogBox">
	...
	<cffunction name="getBinder" access="public" returntype="any" output="false" hint="Get the Injector's configuration binder object" colddoc:generic="wirebox.system.ioc.config.Binder">
	...
	<!--- getScopeStorage --->
    <cffunction name="getScopeStorage" output="false" access="public" returntype="any" hint="Get the scope storage utility" colddoc:generic="wirebox.system.core.collections.ScopeStorage">
	...
	<!--- locateScopedSelf --->
    <cffunction name="locateScopedSelf" output="false" access="public" returntype="any" hint="Return a self reference using the scoped registration, mostly used by providers or scope widening objects" colddoc:generic="wirebox.system.ioc.Injector">
    	<cfscript>
    		var scopeInfo 	= instance.binder.getScopeRegistration();
...
<!----------------------------------------- PRIVATE ------------------------------------->
    ...
    <cffunction name="registerScopes" output="false" access="private" returntype="void" hint="Register all internal and configured WireBox Scopes">
    	<cfscript>
    		...
    		// register no_scope
			instance.scopes["NOSCOPE"] = createObject("component","wirebox.system.ioc.scopes.NoScope").init( this );
			// register singleton
			instance.scopes["SINGLETON"] = createObject("component","wirebox.system.ioc.scopes.Singleton").init( this );
			// is cachebox linked?
			if( isCacheBoxLinked() ){
				instance.scopes["CACHEBOX"] = createObject("component","wirebox.system.ioc.scopes.CacheBox").init( this );
			}
			// CF Scopes and references
			instance.scopes["REQUEST"] 		= createObject("component","wirebox.system.ioc.scopes.RequestScope").init( this );
			instance.scopes["SESSION"] 		= createObject("component","wirebox.system.ioc.scopes.CFScopes").init( this );
    ...
    <cffunction name="configureLogBox" output="false" access="private" returntype="void" hint="Configure a standalone version of logBox for logging">
    	...
    	<cfscript>
			...
			config = createObject("component","wirebox.system.logging.config.LogBoxConfig").init(argumentCollection=args);

			// Create LogBox
			instance.logBox = createObject("component","wirebox.system.logging.LogBox").init( config );
			// Configure Logging for this injector
			instance.log = instance.logBox.getLogger( this );
		</cfscript>
    </cffunction>
    ...
    <cffunction name="configureEventManager" output="false" access="private" returntype="void" hint="Configure a standalone version of a WireBox Event Manager">
    	<cfscript>
			...
			instance.eventManager = createObject("component","wirebox.system.core.events.EventPoolManager").init( instance.eventStates );
			// Debugging
			if( instance.log.canDebug() ){
	...
	<!--- Get ColdBox Util --->
	<cffunction name="getUtil" access="public" output="false" returntype="any" hint="Return the core util object" colddoc:generic="wirebox.system.core.util.Util">
    ...
    <cffunction name="buildBinder" output="false" access="private" returntype="any" hint="Load a configuration binder object according to passed in type">
		...
		<cfscript>
			...
			if( NOT instance.utility.isInstanceCheck(arguments.binder, "wirebox.system.ioc.config.Binder") ){
				// simple data cfc, create native binder and decorate data CFC
				nativeBinder = createObject("component","wirebox.system.ioc.config.Binder").init(injector=this,config=arguments.binder,properties=arguments.properties);
			}
			else{
